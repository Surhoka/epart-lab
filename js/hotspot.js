window.renderHotspots = function (containerId, hotspotsData, onHotspotClick) {
    ...
    hotspotsData.forEach((hotspot, index) => {
        ...
        tooltip.className = 'absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 bg-white dark:bg-gray-800 text-gray-800 dark:text-white text-xs rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-20 p-2 pointer-events-none';
        tooltip.innerHTML = `
            <div class="font-semibold mb-1 border-b border-gray-100 dark:border-gray-700 pb-1">${hotspot.title || 'Detail'}</div>
            <div class="text-gray-500 dark:text-gray-400 leading-tight">${hotspot.content || ''}</div>
            <!-- Arrow -->
