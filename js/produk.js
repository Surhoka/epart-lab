...
function fetchProdukData() {
    ...
    if (typeof window.sendDataToGoogle === 'function') {
        window.sendDataToGoogle('getProduk', {}, (response) => {
            if (response.status === 'success' && response.data) {
                produkData = response.data;
                console.log('Products loaded:', produkData);
                renderProdukTable();
            } else {
 ...
 */
function searchProduk(searchTerm) {
    const tbody = document.getElementById('products-table-body');
    if (!tbody) return;
    ...
    if (typeof window.sendDataToGoogle === 'function') {
        window.sendDataToGoogle('searchProduk', { searchTerm }, (response) => {
            if (response.status === 'success' && response.data) {
                produkData = response.data;
                console.log('Search results:', produkData);
                renderProdukTable();
            } else {
        ...
        <tr>
            <td colspan="6" class="py-8 text-center">
                <div class="flex flex-col items-center gap-2">
                    ...
                    <button onclick="fetchProdukData()" class="mt-2 px-4 py-2 bg-brand-500 text-white rounded-lg hover:bg-brand-600 transition-colors">
                        Retry
                    </button>
                </div>
            </td>
        ...
        window.sendDataToGoogle('deleteProduk', { productId }, (response) => {
            if (response.status === 'success') {
                ...
                if (currentSearchTerm) {
                    searchProduk(currentSearchTerm);
                } else {
                    fetchProdukData();
                }
            } else {
