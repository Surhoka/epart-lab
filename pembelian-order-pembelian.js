document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializePage() {
        // Set tanggal hari ini ke input tanggal
        const today = new Date().toISOString().split('T')[0];
        if (poDateInput) {
            poDateInput.value = today;
        }

        // Generate nomor PO baru (contoh)
        if (poNumberInput) {
            poNumberInput.value = generateNewPONumber();
        }

        // Inisialisasi ikon Lucide jika ada
        if (typeof lucide !== 'undefined') {
            lucide.createIcons();
        }

        console.log('Halaman Order Pembelian siap.');
    }
    ...
    function handleNewOrder() {
        ...
        // Logika untuk mereset form dan menyiapkan order baru
        if (poNumberInput) {
            poNumberInput.value = generateNewPONumber();
        }
        // Reset field lainnya
        ...
        if(poProductList) poProductList.innerHTML = ''; // Kosongkan tabel produk
    ...
    function handleResetTable() {
        ...
        // Logika untuk mereset filter dan mengembalikan data tabel ke kondisi awal
    ...
    function handleAddProduct() {
        ...
        // Ini hanya contoh, idealnya akan membuka modal atau form untuk memilih produk
        const newRowHTML = `
            <tr>
        ...
        if (poProductList) {
            ...
            if (typeof lucide !== 'undefined') {
                lucide.createIcons(); // Re-inisialisasi ikon untuk baris baru
            }
        }
    }
    ...
    function handleSaveOrder(event) {
        ...
        // Logika untuk mengumpulkan data dari form dan tabel, lalu mengirimkannya
    ...
    if (poProductList) {
        poProductList.addEventListener('click', function(e) {
